// commands/unequip.js
const { unequipItem } = require("../core/equip");
...
module.exports = {
    ...
    execute: async (client, channel, user, args) => {
        ...
        const result = unequipItem(user.username, slot);
        ...
        if (result.previous) {
            client.say(channel, `@${user.username} sundal **${result.previous.name}** ze slotu ${slot}.`);
        } else {
            client.say(channel, `@${user.username} slot ${slot} je už prázdný.`);
        }
    }
};
